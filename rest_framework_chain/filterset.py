...
def populate_from_filterset(filter, name, filters):
    """
    ...
    for f in filter.filterset.base_filters.values():
        if _should_skip():
            continue

        f = copy(f)
        filter_key = '%s%s%s' % (name, LOOKUP_SEP, f.name)
        f.name = '%s%s%s' % (filter.name, LOOKUP_SEP, f.name)
        filters[filter_key] = f
